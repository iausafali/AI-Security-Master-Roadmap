...
* **Week 6 (Baseline Functional Verification):**
  * Verify that the agent autonomously executes complex multi-step enterprise workflows in the owned lab.
...
* **Week 9 (Attack Vector 1 — Indirect Prompt Injection → Domain PrivEsc):**
  * Plant a malicious prompt injection inside an employee's SMB file / email body.
  * Trigger agent file analysis in the owned lab; observe the agent parse the file and execute unauthorized administrative commands.
  * Trace debugger state and LLM completion behavior during hijacking.
* **Week 10 (Attack Vector 2 — Kerberoasting via Agent Tool Misuse):**
  * In the owned lab, inject a malicious search query forcing the agent to request TGS tickets for test SPNs and record ticket material for offline analysis.
...
* **Week 12 (Final Artifacts):**
  * `AGENT_SECURITY_LAB_REPORT.md`: Lab topology, Kerberos attack paths, BloodHound graphs, prompt injection PoCs, and defensive capability-token architecture.
...
### Stage 4: Break & Patch
- Execute 3 distinct attack vectors inside the owned lab: Indirect Prompt Injection, Kerberoasting via Agent Tool Abuse, and Exfiltration.
...
- [ ] Comprehensive enterprise security audit report published with BloodHound attack graphs.
